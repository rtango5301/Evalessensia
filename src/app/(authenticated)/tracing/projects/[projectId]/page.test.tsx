...
const project = { id: 'project-1', name: 'Agent project', description: null, created_at: '', trace_count: 1, error_rate: 0, avg_latency_ms: 0, recent_run: null };
const tracePage = { items: [{ trace_id: 'trace-1', status: 'success', latency_ms: 0, total_tokens: 0, total_cost: 0, start_time: null, created_at: '', run_name: 'Root run', run_type: 'agent' }], page: 1, limit: 25, total: 26, pages: 2 };
...
describe('ProjectTracesPage', () => {
  ...
  it('forwards search/status/type filters and pagination', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Search trace ID/), { target: { value: 'needle' } });
    ...
    await waitFor(() => expect(listTraces).toHaveBeenLastCalledWith('project-1', {
      page: 1, limit: 25, search: 'needle', status: 'error', runType: 'tool',
    }));
    fireEvent.click(screen.getByRole('button', { name: 'Next' }));
    await waitFor(() => expect(listTraces).toHaveBeenLastCalledWith('project-1', {
      page: 2, limit: 25, search: 'needle', status: 'error', runType: 'tool',
    }));
  });
});
