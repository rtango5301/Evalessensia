  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        >
          <span className="material-symbols-outlined text-lg">arrow_back</span>Projects
        </Link>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">
          {project?.name || 'Project traces'}
        </h1>
        <p className="mt-1 text-sm text-slate-500">
          Search and inspect every trace recorded for this project.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        <div className="relative flex-1 md:max-w-sm">
          <span className="material-symbols-outlined absolute left-3 top-2.5 text-xl text-slate-400">
            search
          </span>
          <input
            value={search}
            ...
            placeholder="Search trace ID or run name…"
            className="w-full rounded-lg border border-slate-200 bg-white py-2.5 pl-10 pr-3 text-sm outline-none focus:ring-2 focus:ring-[#135bec]"
          />
        </div>
        <select
          value={status}
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-14 text-center">
                    ...
                    <p className="mt-2 text-sm text-slate-500">No traces found</p>
                  </td>
                </tr>
              ) : (
                traces.items.map((trace) => (
                  <tr
                    ...
                    onClick={() => window.location.assign(`/tracing/traces/${trace.trace_id}`)}
                  >
                    <td className="px-6 py-4">
                      <Link
                        href={`/tracing/traces/${trace.trace_id}`}
                        className="text-sm font-semibold text-slate-900 hover:text-[#135bec]"
                      >
                        {trace.run_name || trace.trace_id}
                      </Link>
                      <p className="mt-0.5 font-mono text-xs text-slate-400">{trace.trace_id}</p>
                      <p className="mt-0.5 text-xs capitalize text-slate-500">
                        {trace.run_type || 'custom'}
                      </p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-700">
                      {latencySeconds(trace.latency_ms)}
                    </td>
        ...
        <div className="flex items-center justify-between border-t border-slate-200 px-6 py-4 text-sm text-slate-500">
          <span>
            {traces.total} trace{traces.total === 1 ? '' : 's'}
          </span>
          <div className="flex items-center gap-2">
            ...
            <span>
              Page {page} of {Math.max(traces.pages, 1)}
            </span>
            <button
              disabled={page >= traces.pages || loading}
              onClick={() => setPage((value) => value + 1)}
              className="rounded-lg border border-slate-200 px-3 py-1.5 font-medium disabled:opacity-40"
            >
              Next
