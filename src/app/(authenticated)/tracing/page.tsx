      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Inspect agent runs, execution trees, and runtime details.
          </p>
        </div>
        <button
      ...
      {showCreate && (
        ...
        >
          ...
          <div className="mt-4 flex justify-end gap-3">
            ...
            <button
              disabled={saving}
              className="rounded-lg bg-[#135bec] px-4 py-2 text-sm font-bold text-white disabled:opacity-50"
            >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-blue-700">
              Use your existing Evalessensia API key and set the project name in your agent
              environment.
            </p>
          </div>
          <Link
            href="/dashboard/settings"
            className="w-fit rounded-lg border border-blue-200 bg-white px-4 py-2 text-sm font-bold text-[#135bec] hover:bg-blue-50"
          >
            Manage API Keys
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="px-6 py-14 text-center">
            ...
            <p className="mt-1 text-xs text-slate-400">
              Create one here or send your first SDK trace.
            </p>
          </div>
        ) : (
                ...
                {projects.map((project) => (
                  <tr key={project.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500">
                      {project.recent_run ? new Date(project.recent_run).toLocaleString() : '—'}
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-700">{project.trace_count}</td>
                    <td className="px-6 py-4 text-sm text-slate-700">
                      {(project.error_rate * 100).toFixed(1)}%
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-700">
                      {latencySeconds(project.avg_latency_ms)}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2">
                        <Link
                          href={`/tracing/projects/${project.id}`}
                          className="rounded-lg px-3 py-2 text-sm font-bold text-[#135bec] hover:bg-blue-50"
                        >
                          View traces
                        </Link>
                        <button
