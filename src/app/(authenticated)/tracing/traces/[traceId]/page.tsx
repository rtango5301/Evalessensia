  ...
  return (
    <div>
      <button
        ...
        className={`flex w-full items-center gap-2 border-l-2 py-2.5 pr-3 text-left text-sm transition-colors ${selectedId === node.id ? 'border-[#135bec] bg-blue-50 text-[#135bec]' : 'border-transparent text-slate-600 hover:bg-slate-50'}`}
      >
        <span className="material-symbols-outlined text-lg">
          {node.children.length ? 'account_tree' : 'subdirectory_arrow_right'}
        </span>
...
export default function TraceDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getTrace(traceId), getTraceTree(traceId)])
      ...
      .catch((caught) => {
        if (active) setError(caught instanceof Error ? caught.message : 'Failed to load trace');
      });
    return () => {
      active = false;
    };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <Link
          href={`/tracing/projects/${trace.project_id}`}
          className="mb-3 inline-flex items-center gap-1 text-sm font-medium text-slate-500 hover:text-[#135bec]"
        >
          <span className="material-symbols-outlined text-lg">arrow_back</span>Project traces
        </Link>
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Trace detail</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ['Latency', latencySeconds(trace.latency_ms)],
          ['Tokens', String(trace.total_tokens ?? 0)],
          ['Cost', `$${(trace.total_cost ?? 0).toFixed(4)}`],
          ['Started', trace.start_time ? new Date(trace.start_time).toLocaleString() : '—'],
        ].map(([label, value]) => (
      ...
      <div className="grid min-h-[560px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="border-b border-slate-200 lg:border-b-0 lg:border-r">
          <div className="border-b border-slate-200 px-4 py-3">
            <h2 className="text-sm font-bold text-slate-900">Execution tree</h2>
          </div>
          <div className="max-h-[650px] overflow-auto py-2">
            <TreeItem node={tree} selectedId={selected.id} onSelect={setSelected} />
          </div>
        </aside>
