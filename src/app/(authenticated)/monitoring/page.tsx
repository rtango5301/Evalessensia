...
export default function MonitoringPage() {
  ...
  const totals = useMemo(
    () => ({
      ...
      latency: metrics.latency.length
        ? metrics.latency.reduce((sum, value) => sum + value, 0) / metrics.latency.length / 1000
        : 0,
    }),
    [metrics]
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Track trace volume, errors, latency, tokens, and cost over time.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ['Traces', totals.traces.toLocaleString(), 'route'],
          ['Errors', totals.errors.toLocaleString(), 'error'],
          ['Avg latency', `${totals.latency.toFixed(2)} s`, 'timer'],
          ['Tokens', totals.tokens.toLocaleString(), 'token'],
          ['Cost', `$${totals.cost.toFixed(4)}`, 'payments'],
      ...
      {loading ? (
        <div className="h-96 animate-pulse rounded-xl border border-slate-200 bg-white p-6">
        ...
        <>
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-5">
              <h2 className="font-bold text-slate-900">Trace health</h2>
              <p className="mt-1 text-xs text-slate-500">
                Volume, errors, and average latency in seconds
              </p>
            </div>
            <div className="h-80">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-5">
              <h2 className="font-bold text-slate-900">Usage</h2>
              <p className="mt-1 text-xs text-slate-500">Token consumption and cost</p>
            </div>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={rows}>
